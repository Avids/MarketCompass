 ...
 */
export function buildTickerData(dates, closes, spyCloses, lookback = 126) {
    // Calculate rolling MAs over the full series first (so early lookback values are accurate)
    const sma20Full = calcSMA(closes, 20);
    const sma50Full = calcSMA(closes, 50);

    // MA Spread: % deviation from 50-day SMA
    const spreadFull = closes.map((c, i) =>
        sma50Full[i] !== null ? parseFloat(((c - sma50Full[i]) / sma50Full[i] * 100).toFixed(3)) : null
    );
