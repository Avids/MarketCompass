...
/**
 * Calculate an exponential moving average (EMA) over an array of numbers.
 * Returns null for positions where there aren't enough data points yet.
 */
export function calcEMA(values, period) {
    ...
    let currentEma = firstSlice.reduce((a, b) => a + b, 0) / period;
    ema[period - 1] = parseFloat(currentEma.toFixed(3));

    const k = 2 / (period + 1);
    for (let i = period; i < values.length; i++) {
        currentEma = (values[i] * k) + (currentEma * (1 - k));
        ema[i] = parseFloat(currentEma.toFixed(3));
    }
    return ema;
}
 ...
 */
export function buildTickerData(dates, closes, spyCloses, lookback = 126, timeframe = 'daily', volumes = null, highs = null, lows = null) {
    // Calculate rolling MAs over the full series first (8/21 as EMAs, rest as SMAs)
    const ema8Full = calcEMA(closes, 8);
    ...
    for (let i = 0; i < closes.length; i++) {
        const c = closes[i];
        const h = highs && highs[i] !== undefined ? highs[i] : c;
        const l = lows && lows[i] !== undefined ? lows[i] : c;
        const v = volumes && volumes[i] !== undefined ? volumes[i] : 0;
        let mfm = 0;
        if (h !== l) {
            mfm = ((c - l) - (h - c)) / (h - l);
        }
        runningAd += mfm * v;
        adFull.push(runningAd);
    }
    ...
    const trimEma8    = ema8Full.slice(start);
    const trimEma21   = ema21Full.slice(start);
    ...
    const trimAd      = adFull.slice(start);
    ...
    // Z-score series for the spread: (value - mean) / std
    const zscore = trimSpread.map(v => (v !== null && std !== 0) ? parseFloat(((v - mean) / std).toFixed(3)) : null);
    // Last non-null z-score (most recent)
    const lastZ = (() => {
        for (let i = zscore.length - 1; i >= 0; i--) {
            if (zscore[i] !== null) return zscore[i];
        }
        return null;
    })();
    ...
    return {
        ...
        ma8:             trimEma8.map(m => m !== null ? parseFloat(m.toFixed(2)) : null),
        ma21:            trimEma21.map(m => m !== null ? parseFloat(m.toFixed(2)) : null),
